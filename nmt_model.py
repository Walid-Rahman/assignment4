...
class NMT(nn.Module):
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        #Retrive  target embedding
        Y = self.model_embeddings.target(target_padded)
        #Split Y
        Y_t  = torch.split(Y, 1, dim=0)
        for i in Y_t:
            #squeeze and concatenation
            Ybar_t = torch.cat((torch.squeeze(i, 0), o_prev), 1)
            #applied step
            dec_state, o_t, e_t = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            #Append o_t to combined_outputs
            combined_outputs.append(o_t)
            #update o_prev with o_t
            o_prev = o_t
        #convert combined_outputs from a list length tgt_len of tensors shape (b, h), to a single tensor shape (tgt_len, b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ##squeeze third dimension  e_t is (b,src_len) and enc_hiddens_proj is shape (b, src_len, h),dec_hidden is shape (b, h)
